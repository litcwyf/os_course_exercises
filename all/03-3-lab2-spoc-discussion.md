...
特权指令只能在level0（ring0）中使用；一条指令在不同特权级别下的访问权限不同。


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。

访问中断门、陷入门时，需要判断当前代码段的CPL小于门所处于的DPL，且CPL要大于等于段的DPL；访问某个段时，要求当前段CPL和RPL的最大值小于等于要访问的段的DPL。
...
低特权级的程序需要访问高特权级的数据的时候

3. int指令在ring0和ring3的执行行为有什么不同？

在ring0的时候，需要多在栈中保存SS和ESP，还需要进行栈的切换

4. 如何利用int和iret指令完成不同特权级的切换？

人工构造一个栈的结构，然后通过iret指令完成转换。
...
根据虚拟地址从段描述符中提取出基地址，再加上偏移得到线性地址；通过线性地址查找页表得到物理页号，结合偏移得到物理地址。

### 7.4 了解UCORE建立段/页表

1. 分析MMU的使能过程，尽可能详细地分析在执行进入保护械的代码“movl %eax, %cr0 ; ljmp $CODE_SEL, $0x0”时，CPU的状态和寄存器内容的变化。

首先，在kernel_init的时候建立GDT，然后使能页机制（将CR0的PG寄存器的31位置为1），最后，执行ljmp的指令。

2. 分析页表的建立过程；

1、根据内存分配算法，分配一个4K的空间作为当前页目录的table，并清空内容（初始化）；2、在对应的页目录表和页表中，填写好相应的表项；3、CR0的寄存器的31位置1；4、更新GDT
